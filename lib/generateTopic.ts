...
export function generateTopic(
  ...
  // 주제 제목 생성
  const topicTitle = `${keywordStr} 기반 ${isApp ? "모바일 앱" : "웹 서비스"}: ${selectedNodes[0]?.title || "통합 솔루션"}`;
  
  // 설명 생성
  const description = `${keywordStr}를 중심으로 한 ${isApp ? "모바일 앱" : "웹 서비스"}입니다. ${selectedNodes.length}개의 핵심 아이디어를 통합하여 사용자에게 차별화된 가치를 제공합니다. ${nodeSummaries.slice(0, 100)}...`;
  ...
  // 차별화 포인트
  const uniqueValue = `${selectedNodes.length}개의 핵심 아이디어를 통합한 ${isApp ? "앱" : "서비스"}로, ${keywordStr} 분야에서 독특한 접근 방식을 제공합니다.`;
